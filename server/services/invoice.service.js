...
const InvoiceHeaderDao = require("../data_access_layer/daos/invoice_header.dao");
...
exports.getBilledLast12Months = async (beginDate, finishDate, yearMonthList) => {
    let billedList = [];

    return new Promise(async (resolve, reject) => {
        await InvoiceHeaderDao.getInvoicesByDate(beginDate, finishDate).then(async data => {
            if (data) {
                finishDate.setMonth(finishDate.getMonth() - 11);
                ...
                yearMonthList.forEach(ym => {
                    let billed = 0;
                    data.forEach(e => {
                        if (e['INVOCIE_DATE'] >= beginDate && e['INVOCIE_DATE'] < finishDate) {
                            billed += e['FOREIGN_CURR_VALUE'];
                        }
                    });
...
exports.getAverages = async (startDate, endDate) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // Get the total dues for each month
        ...
        await this.getBilledLast12Months(beginDate, finishDate, yearMonthList).then(async data => {
            billedList = data;
        }).catch(err => {
            reject(err.message);
        });
