...
exports.createChartReportData = async (createdChartReport, data) => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const isEmpFiltered = data[i].label.toString().split(' - ')[1] === 'emp'
      const year = parseInt(data[i].label.toString().split(' - ')[0])
      const set = {
        ...
        if (data.recipients) {
          for (let i = 0; i < data.reportTypes.length; i++) {
            for (let j = 0; j < data.recipients.length; j++) {
              const recipientId = data.recipients[j].recipientId
      ...
      .then(async data => {
        if (data) {
          ...
          await this.getChartReportPDFAverages(reportId)
            .then(async (dataAvg) => {
              if (dataAvg) { averagesList = dataAvg } else { resolve(false) }
            }).then(() => {
