...
exports.getChartReportsByUserId = async (req, res) => {
  if (!req.user || !req.user.userId || req.user.userId === '' || req.user.userId === undefined) { return res.status(400).send({ message: 'Content cannot be empty.' }) }
...
exports.createChartReport = async (req, res) => {
  if (!req.user || !req.user.userId || req.user.userId === '' ||
        ...
        req.body.chartReportData === undefined) { return res.status(400).send({ message: 'Content cannot be empty.' }) }

  const chartReportCriteria = req.body.chartReport
  const chartReportData = req.body.chartReportData
...
exports.getReportTypesWithRecipients = async (req, res) => {
  ...
  if (req.user.role !== 'admin') { return res.status(403).send({ message: 'You are not authorized to fetch from this resource.' }) }
...
exports.createChartReportPDF = async (req, res) => {
  if (!req.body.reportid) { return res.status(400).send({ message: 'Content cannot be empty.' }) }
...
exports.fetchChartReportPDF = async (req, res) => {
  ...
  if (!req.query.reportid) { return res.status(400).send({ message: 'Content cannot be empty.' }) }
