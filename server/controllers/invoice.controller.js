...
exports.getAverages = async (req, res) => {
  const regexDateStr = /^\d{4}-\d{2}-\d{2}$/
  const regexDate = new RegExp(regexDateStr)

  if (!req.params.startDate || !req.params.endDate) { return res.status(400).send({ message: 'Content cannot be empty.' }) }

  if (!regexDate.test(req.params.startDate) || !regexDate.test(req.params.endDate)) { return res.status(400).send({ message: 'Wrong format.' }) }
