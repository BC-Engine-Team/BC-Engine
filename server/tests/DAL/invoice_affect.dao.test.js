...
describe('Test Invoice Affect DAO', () => {
  const fakeQuery = { queryString: 'fakeQuery', replacements: ['fakeReplace'] }
  const prepareBilledQuerySpy = jest.spyOn(InvoiceAffectDao, 'prepareBilledQuery')
  ...
  const clientType = 'DIRECT'
  const countryCode = 'CA'
  const ageOfAccount = '<30'
  ...
  describe('IAD2 - prepareBilledQuery', () => {
    ...
    describe('IAD2.1 - given startDate and endDate', () => {
      it('IAD2.1.1 - should respond with query only filtering by date', async () => {
        ...
        const expectedResponse = expectedQuery
