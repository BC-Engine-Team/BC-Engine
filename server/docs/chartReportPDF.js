module.exports = (data, averagesList) => {
  ...
  const buildChartDatasets = () => {
    ...
    for (let i = 0; i < averagesList.length; i++) {
      ...
      if (counterCompare === 5) { counterCompare = 0 }
      ...
      for (let j = 0; j < averagesList[i].data.length; j++) {
        ...
        if (j + 1 !== averagesList[i].data.length) { str = str.concat(',') } else { str = str.concat('],') }
      }
      ...
      if (i + 1 !== averagesList.length) { str = str.concat(',') }
    }
  ...
  const buildTable = () => {
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      for (let j = 0; j < averagesList.length; j++) {
        ...
        if (compareCounter === 5) { compareCounter = 0 }
