...
const Dashboard = () => {
  ...
  const chart = async (compare = false) => {
    ...
    for (let c = 0; c < arrayLength; c++) {
      ...
      const startDate = new Date(criteria.startYear, criteria.startMonth, 1).toISOString().split('T')[0]
      const endDate = new Date(criteria.endYear, criteria.endMonth, 1).toISOString().split('T')[0]
      ...
      await Axios.get(`${process.env.REACT_APP_API}/invoices/defaultChartAndTable/${startDate}/${endDate}`, { params: param, headers: header })
        .then((res) => {
          ...
          const datasets = []
          const groupedChartData = res.data[0].chart
          ...
          for (let i = 0; i < Object.keys(groupedChartData).length; i++) {
            ...
            const startMonth = parseInt(dataGroup[0].month.toString().substring(4)) - 1
            const endMonth = parseInt(dataGroup[dataGroup.length - 1].month.toString().substring(4)) - 1
            let counter = 0
  ...
  const findCriteriaErrors = () => {
    ...
    // name errors
    if (name.length === 0) { newErrors.name = t('error.Empty') }

    // endYear errors
    if (parseInt(endYear) < parseInt(startYear)) { newErrors.endYear = t('dashboard.criteria.EndYearExceedError') }

    // endMonth errors
    if (parseInt(endMonth) < parseInt(startMonth) && parseInt(startYear) === parseInt(endYear)) { newErrors.endMonth = t('dashboard.criteria.EndMonthExceedError') }

    if (parseInt(endMonth) > parseInt(currentMonth) && parseInt(endYear) === parseInt(currentYear.toString())) { newErrors.endMonth = t('dashboard.criteria.EndMonthExceedCurrentError') }

    // startMonth errors
    if (startMonth > currentMonth && startYear === currentYear.toString()) { newErrors.startMonth = t('dashboard.criteria.StartMonthExceedCurrentError') }
  ...
  const handleSaveChartReport = async (event) => {
    ...
    const previouslyLoadedCriteria = JSON.parse(previouslyLoadedCriteriaStr)
    delete previouslyLoadedCriteria.name
    const criteriaWithoutName = Object.assign({}, criteria)
      ...
      <div className='main-container main-div'>
        <div className='justify-content-center main'>
          <div className='container-criteria'>
            <div className='card shadow my-3 mx-3 px-3 py-2'>
              ...
              <Row>
                ...
                <InputGroup className='mb-2'>
                  ...
                  >
                    ...
                    <Form.Select
                      id='employeeCriteriaDashboard' onChange={(e) => {
                        setField('employee1', {
                          id: e.target.value,
                          name: e.target.options[e.target.selectedIndex].text
                        })
                  ...
                  >
                    ...
                    <Button
                      ...
                      className='btn btn-light shadow-sm border input-select ms-1'
                    >
              ...
              <Row>
                ...
                <InputGroup className='mb-2'>
                  <Form.Select
                    id='clientTypeCriteriaDashboard' onChange={(e) => {
              ...
              <Row>
                ...
                <InputGroup className='mb-2'>
                  <Form.Select
                    id='ageOfAccountCriteriaDashboard' onChange={(e) => {
          ...
          <div className='container-table'>
            <div className='card shadow my-3 mx-3 user-table'>
              ...
              <ButtonGroup className='col-md-5 w-auto ms-auto client-swap'>
                ...
                <DropdownButton title={t('dashboard.dropdownButtonTable.Title')} variant='Default' id='bg-vertical-dropdown-3' className='rows-viewer-selection-style'>
