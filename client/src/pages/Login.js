      ...
      Axios.post(`${process.env.REACT_APP_API}/users/authenticate`, data)
        .then((response) => {
          if (response.data.auth === true) {
            const aToken = response.data.aToken.toString()
            const rToken = response.headers.authorization.toString()
            const username = response.data.authenticatedUser.name.toString()
            const role = response.data.authenticatedUser.role.toString()
            const userId = response.data.authenticatedUser.userId.toString()
        ...
        <div id='login-card' className='card shadow p-3 m-5'>
          ...
          >
            ...
            <Form.Group className='mb-5' controlId='floatingPassword'>
              <FloatingLabel controlId='floatingPassword' label={t('form.Password')} className='mb-3 input-with-show-hide'>
                <Form.Control
                  required
                  className='password-validation-field'
            ...
            <div className='d-flex justify-content-center mt-5 mb-4'>
              <Button
                id='loginButton'
                type='submit'
                className='btn btn-light py-2 px-5 my-1 shadow-sm border submit-button'
              >
